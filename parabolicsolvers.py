...
#*******************************************************************************
def FTCS(InputSettings, BCType='Dirichlet'):
    '''Solve a 2D parabolic partial differential equation using the explicit 
    ...
    dX = ri.Length/(ri.iMax - 1)
    dY = ri.Height/(ri.jMax - 1)

    # Initialize fields everywhere at t = 0
    U = pre.Initial(ri.iMax, ri.jMax)
    
    # Assign boundary conditions 
    U = pre.BC(U, ri.iMax, ri.jMax, BCType)
    ...
    while Error > ConvCriteria and n < nMax: # Start iteration
        ...
        for i in range (1, ri.iMax-1):
            for j in range (1, ri.jMax-1):
                Uold = U[i][j]
                U[i][j] = Uold\
                          + diffX*(U[i+1][j] - 2*U[i][j] + U[i+1][j])\
                          + diffY*(U[i][j+1] - 2*U[i][j] + U[i][j-1]) # Alternating Direction Implicit SOR method in y-direction
        ...
        U = pre.BC(U, ri.iMax, ri.jMax, BCType) # Update boundary conditions
        ...
        if n % nWrite == 0:
            post.WriteSolutionToFile(OutFileName, ri.iMax, ri.jMax, dX, dY, U)
...
#*******************************************************************************
def DuFortFrankel(InputSettings, BCType='Dirichlet'):
    '''Solve a 2D parabolic partial differential equation using the
